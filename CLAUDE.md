...
### 形を「見る」のではなく「読む」
- スクリーンショットは知覚ではない。画像認識はR15とR14を区別できないし、フィレットが面を丸ごと食ったことにも気づかない。**形が知りたいときは `describe_shape` を使う**（実測から算出: 寸法・bbox充填率・面の内訳・開口部・対称面）
- 作ったときに自分が認識していた形は `shape` に書く（例: 「小判形の板 40×30×10、四隅R15で側面は全て円弧、中央にφ6貫通」）。次に開いた者は、画像ではなくこの一文と `provenance`（構築レシピ）で形を把握する
- **ただし形状の散文は次の編集で古くなる。** `shape` を書くと実測の指紋（体積・寸法・面数・エッジ数・開口数）が自動で刻まれ、`check_context_integrity` の `stale_shapes` が「その記述はもう別の物体を指している」と報告する。位置を動かしただけでは古くならない（形の記述だから）
- モジュールにも `shape` を書ける。各ボディは自分の形を知っているが、**合わさって何になるかは誰も知らない**
- **加工したら形を書き直す。** 「円柱＋中心に穴あけ」は手順であって形ではない。できあがったものを名指しする — 「外形φ20・内径φ16、肉厚2mmのパイプ」。`add_fillet` などの加工ツールに `shape` を渡せば、その場で貼り替わる（指紋も更新される）
- 複雑になって一文で書けなくなったら、無理に書かず `provenance` を辿る。**消費されて消えた部品の理由も、吸収したボディの provenance に残る**（`consumed` 付きの行）ので、「この穴は何のためか」は答えられる

### 意図の行き先（生成と加工で違う）
- **生成**（create_*, copy_body）の `intent` → そのボディの存在理由になる
- **加工**（fillet, chamfer, hole, boolean, cut）の `intent` → `provenance` の行に載る。**ボディの存在理由は上書きされない**（「なぜこの面取りか」が「なぜこの部品が在るか」を潰さないため）
- `shape` / `constraints` / `placement` / `dimensions` は加工時でもボディ側に載る（加工後の再記述はここ）
- provenance は20件で頭打ちだが、**理由の書かれた行は落ちない**。半径は形から読み直せるが、なぜ付けたかは読み直せないため
